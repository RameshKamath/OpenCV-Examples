...
cv::Mat compute_median(std::vector<cv::Mat> vec)
{
	...
	for (int row = 0; row < vec[0].rows; row++)
	{
		for (int col = 0; col < vec[0].cols; col++)
		{
			...
			//seperate each channel to calculate median seperately
...
int main()
{
	...
	// Convert median to gray scale for comparision
	...
	// set Named Window
	cv::namedWindow("Extracted Dynamic Features", cv::WINDOW_NORMAL);
	...
	while (cap.isOpened())
	{
		...
		if (cap.grab())
		{
			...
			// Convert frame to gray scale for comparision
			cv::cvtColor(frame, grayFrame, cv::COLOR_BGR2GRAY);

			// find difference to get dynamic features
			cv::absdiff(grayFrame, grayMedianFrame, Difference);

			// threshold to remove noise
			cv::threshold(Difference, Difference, 30, 255, cv::THRESH_BINARY);

			// Display Dynamic Features frame
			imshow("Extracted Dynamic Features", Difference);
